...
export class PostagemService {
    ...
    public manterPostagem(request: Request, response: Response) {
        ...
        if(postagem.isPostValido()){
            ...
            this.db.doc(`publicacoesmatheus/${postagem.id}`).set(postagem.toJson(), {merge: true})
            .then(resultadoSnap => {
    ...
    public comentarPostagem(request: Request, response: Response) {
        ...
        if(idPostagem === undefined || idPostagem === ""){
            ...
            this.db.doc(`publicacoesmatheus/${idPostagem}`).get()
            .then(postSnap => {
    ...
    public excluirPostagem(request: Request, response: Response) {
        if(request.query.id === undefined || request.query.id === ""){
            ...
            this.db.doc(`publicacoesmatheus/${request.query.id}`).delete()
            .then(_ => {
    ...
    public excluirComentario(request: Request, response: Response){
        ...
        if(idPostagem === undefined || idPostagem === "" ||
        idComentario === undefined || idComentario === ""){
            HttpUtil.falha("Post ou comentário inválido", response)
        }
        else{
            this.db.doc(`publicacoesmatheus/${idPostagem}`).get().then(postSnap => {
    ...
    public listarPosts(request: Request, response: Response){
        this.db.collection("publicacoesmatheus").orderBy("dataDePostagem", "desc").get().then(postsSnap => {
        ...
        }).catch(erro => {HttpUtil.falha("Houve um erro ao listar os posts" + erro, response)})
    }
}